...
"Times New Roman","serif"'>Tipe Dianix<o:p></o:p></span></p>
...
<div class="pull-right">
   <div class="empty">
		   ...
		   <script type="text/javascript">
            ...
            normal'><span style='font-size:12.0pt;mso-bidi-font-size:11.0pt;font-family:
           "Times New Roman","serif"'>
			<a id="return" data-placement="top" class="btn btn-success" title="Click to Return" href="damage_keyboard.php"><i class="icon-arrow-left"></i> Kembali</a></p>		
			<script type="text/javascript">
			$(document).ready(function(){
			$('#return').tooltip('show');
			$('#return').tooltip('hide');
...
<?php
		$device_query = mysql_query("select * from stdevice			
							 ...
							 where dev_name = 'dianix' and dev_status = 'terpakai' ORDER BY location_details.ld_id DESC")or die(mysql_error());
...
<table class=MsoTableGrid border=1 cellspacing=0 cellpadding=0
 ...
 <?php	
			 ...
			 where NOT EXISTS 
	             ...
	             and dev_status = 'terpakai' and dev_name = 'dianix'
		        OR NOT EXISTS 
	            (select * from location_details where stdevice.id = location_details.id)
		        and dev_status = 'terpakai' and dev_name = 'DIANIX'
		        OR NOT EXISTS 
              ...
              (select * from location_details where stdevice.id = location_details.id)
		        ...
		        OR NOT EXISTS 
	            (select * from location_details where stdevice.id = location_details.id)
		        and dev_status = 'terpakai' and dev_name = 'dianx'
		        OR NOT EXISTS 
	            (select * from location_details where stdevice.id = location_details.id)
		        and dev_status = 'terpakai' and dev_name = 'dianx'
		        OR NOT EXISTS 
	            (select * from location_details where stdevice.id = location_details.id)
		        and dev_status = 'terpakai' and dev_name = 'dinx'
		        OR NOT EXISTS 
	            (select * from location_details where stdevice.id = location_details.id)
		        and dev_status = 'terpakai' and dev_name = '(dianix)'
		        OR NOT EXISTS 
...
<?php	
	              $count_item=mysql_query("select * from stdevice 
         LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
         where dev_name = 'dianix' And dev_status = 'Terpakai' ORDER BY stdevice.id DESC ");
               $count = mysql_num_rows($count_item);
 ?>	 
