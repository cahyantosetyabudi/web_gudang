...
"Times New Roman","serif"'>Tipe Buku Kwitansi<o:p></o:p></span></p>
...
<div class="pull-right">
   <div class="empty">
		   ...
		   <script type="text/javascript">
            ...
            normal'><span style='font-size:12.0pt;mso-bidi-font-size:11.0pt;font-family:
           "Times New Roman","serif"'>
			<a id="return" data-placement="top" class="btn btn-success" title="Click to Return" href="damage_power_supply.php"><i class="icon-arrow-left"></i> Kembali</a></p>		
			<script type="text/javascript">
			$(document).ready(function(){
			$('#return').tooltip('show');
			$('#return').tooltip('hide');
...
<?php
		$device_query = mysql_query("select * from stdevice			
							 ...
							 where dev_name = 'Buku Kwitansi' And dev_status = 'rusak' ORDER BY location_details.ld_id DESC")or die(mysql_error());
 ...
 <?php	
			 ...
			 where NOT EXISTS 
	          (select * from location_details where stdevice.id = location_details.id)	   
	           and dev_status = 'rusak' and dev_name = 'Buku Kwitansi'
		      OR NOT EXISTS 
	          (select * from location_details where stdevice.id = location_details.id)
		      and dev_status = 'rusak' and dev_name = 'BUKU KWITANSI'
		      OR NOT EXISTS 
		      ...
		      OR NOT EXISTS 
	          (select * from location_details where stdevice.id = location_details.id)
		      and dev_status = 'rusak' and dev_name = 'bkukwitnsi'
		      OR NOT EXISTS 
	          (select * from location_details where stdevice.id = location_details.id)
		      and dev_status = 'rusak' and dev_name = 'bukkwin'
		      OR NOT EXISTS 
	          (select * from location_details where stdevice.id = location_details.id)
		      and dev_status = 'rusak' and dev_name = 'bkkwitansi'
		       OR NOT EXISTS 
	          (select * from location_details where stdevice.id = location_details.id)
		      and dev_status = 'rusak' and dev_name = 'bkkwtnsi'
		      OR NOT EXISTS 
	          (select * from location_details where stdevice.id = location_details.id)
		      and dev_status = 'rusak' and dev_name = 'buku'
		       OR NOT EXISTS 
	           (select * from location_details where stdevice.id = location_details.id)
		       and dev_status = 'rusak' and dev_name = 'kwitansi'
		       OR NOT EXISTS 
	           (select * from location_details where stdevice.id = location_details.id)
		       and dev_status = 'rusak' and dev_name = 'bku'
		       OR NOT EXISTS 
	          (select * from location_details where stdevice.id = location_details.id)
		       and dev_status = 'rusak' and dev_name = 'kwtnsi'
...
<?php	
	             $count_item=mysql_query("select * from stdevice 
				 LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
				 where dev_name = 'Buku Kwitansi' And dev_status = 'rusak' ORDER BY stdevice.id DESC ");
	             $count = mysql_num_rows($count_item);
 ?>	 
